...
`,
    fn: async (cwd, cmd, args, opts) => {
        ...
        if (release) {
            new_ver = semver.inc(new_ver, release, 'beta')
            if (!new_ver && semver.valid(release)) {
                new_ver = release
            }
        }

        if (!new_ver) {
            die('Could not resolve version')
        }
        ...
        for (const x of candidates) {
            ...
            for (const dep of x.dependents) {
                ...
                log.info(
                    `[${xpkg.name}] ${dpkg.name}: ${xpkg.name}@${
                        dpkg.dependencies[xpkg.name]
                    } => ${bold(new_ver)}`
                )
